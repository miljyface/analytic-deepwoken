...
def build_mantra_embed(mantra: dict) -> discord.Embed:
    ...
    attributes = ', '.join(payload.get('attribute', [])) if payload.get('attribute') else 'None'
    ...
    if attunement_reqs != 'None':
        embed.add_field(name="Attunement Requirement", value=attunement_reqs, inline=False)
    if base_reqs != 'None':
        embed.add_field(name="Base Requirement", value=base_reqs, inline=False)
    if weapon_reqs != 'None':
        embed.add_field(name="Weapon Requirement", value=weapon_reqs, inline=False)

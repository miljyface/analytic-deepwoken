...
def comparison(id):
    build1 = build(id)
    ordered = order(build1.pre.copy(),{"Race":build1.race, "PointsSpent":0})
    reinvest = 0

    print("\nShrine Process:")
    for k, v in ordered.items():
        if ordered[k] != 0:
            print(f"{k}: {build1.flatpre[k]} -> {v}")
    print("\nProgression:")
    for k, v in ordered.items():
        if ordered[k] != 0:
            print(f"{k}: {v} -> {build1.flatpost[k]}, Change: {build1.flatpost[k]-v}")
            if build1.flatpost[k]-v <= 50:
                reinvest += build1.flatpost[k]-v
    print(f"\nTotal reinvested points: {reinvest}")
